...
- **Vers le tableau de bord** : sur agenda-five-sigma.vercel.app (ou
  localhost), les données fraîches sont poussées directement dans la page, au
  même format que `data.json`. Les identifiants (`id`) sont calculés avec les
  mêmes graines que le scraper local : les deux collecteurs coexistent sans
  doublon, et les résumés d'agent déjà présents survivent.
...
`*.omnivox.ca` ; rien n'est envoyé à aucun serveur — tout reste dans
`chrome.storage.local` de ton navigateur.
...
| `omnivox.js` | Content script Omnivox : moisson passive (tous les cadres) |
| `background.js` | Service worker : fusion, alarme horaire, santé de session |
| `offscreen.html/js` | Analyse du HTML relu en arrière-plan (le SW n'a pas de DOM) |
| `bridge.js` | Content script du tableau de bord : livraison des données |
| `popup.html/js` | État de la collecte + « Collecter maintenant » |
